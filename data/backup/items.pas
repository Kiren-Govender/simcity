...
function    FindbyName( aoid: String): integer;
end;
...
function TItemList.FindbyName( aoid: String): integer;
begin
if self.Count > 0 then
self.Clear;

Params.Clear;
AddParam('aoid', 'aoid', ptString, aoid);
self.SQL := 
' SELECT * FROM item WHERE oid = :aoid;'; 
...
lParam := TSelectParam(lList.Params.FindByProps(['ParamName'], ['aoid']));
Query.ParamAsString['aoid'] := lParam.Value;
end;
