...
Forder_type_index: Integer;
procedure Setorder_type_name(const AValue: String); virtual;
procedure Setorder_type_index(const AValue: Integer); virtual;
public
procedure   Read; override;
procedure   Save; override;
published
property    order_type_name: String read Forder_type_name write Setorder_type_name;
property    order_type_index: Integer read Forder_type_index write Setorder_type_index;
end;
...
{ TOrderTypeList_GetLastOrderNumberVis }
TOrderTypeList_GetLastOrderNumberVis = class(TtiMapParameterListReadVisitor)
protected
function    AcceptVisitor: Boolean; override;
procedure   MapRowToObject; override;
procedure   SetupParams; override;
...
GTIOPFManager.ClassDBMappingMgr.RegisterMapping(TOrderType,
'order_type','order_type_index', 'order_type_index');
...
procedure TOrderType.Setorder_type_index(const AValue: Integer);
begin
if Forder_type_index <> AValue then
Forder_type_index := AValue;
end;
...
function TOrderTypeList.GetLastOrderNumber: integer;
begin
if self.Count > 0 then
self.Clear;
...
Query.ParamAsInteger['order_type_index'] := lObj.order_type_index;
end;
...
' order_type_index = :order_type_index ' + 
'WHERE OID = :OID' ;
end;
...
{ TOrderTypeList_GetLastOrderNumberVis }
function TOrderTypeList_GetLastOrderNumberVis.AcceptVisitor: Boolean;
begin
result := (Visited.ObjectState = posEmpty);
end;

procedure TOrderTypeList_GetLastOrderNumberVis.MapRowToObject;
var
lObj: TOrderType;
begin
lObj := TOrderType.Create;
